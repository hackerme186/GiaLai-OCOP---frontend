...
export default function WalletPage() {
    ...
    const loadData = async () => {
        try {
            ...
            const [walletData, transactionsData, requestsData, bankAccountsData] = await Promise.all([
                ...
                getBankAccounts().catch(() => []) // Bank accounts might not be available
            ])
            ...
            const defaultAccount = bankAccountsData.find(acc => acc.isDefault)
            if (defaultAccount) {
    ...
    const handleDeposit = async () => {
        ...
        try {
            ...
            setSuccess("Yêu cầu nạp tiền đã được gửi. Vui lòng quét QR code để chuyển tiền.")
        } catch (err) {
            console.error("Failed to create deposit request:", err)
            setError(err instanceof Error ? err.message : "Không thể tạo yêu cầu nạp tiền")
        } finally {
            setSubmitting(false)
        }
    }
    ...
    const handleWithdraw = async () => {
        ...
        try {
            ...
            setSuccess("Yêu cầu rút tiền đã được gửi. Vui lòng chờ SystemAdmin phê duyệt.")
    ...
    const handleSaveBankAccount = async () => {
        ...
        try {
            ...
            console.error("Error details:", err.response || err.bodyMessage || err.message)
    ...
    const handleSetDefaultBankAccount = async (id: number) => {
        try {
            ...
            setError(err instanceof Error ? err.message : "Không thể đặt tài khoản mặc định")
        }
    }
            ...
            <>
                ...
                <div className="min-h-screen bg-gradient-to-br from-gray-50 via-indigo-50 to-purple-50 flex items-center justify-center">
                    <div className="text-center">
                        ...
                        <p className="text-gray-600 text-lg font-medium">Đang tải thông tin ví...</p>
                    </div>
                </div>
                <Footer />
            </>
        ...
        <>
                        ...
                        <div className="relative z-10">
                            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                                <div className="flex-1">
                                    <p className="text-indigo-100 text-sm mb-2 font-medium">Số dư hiện tại</p>
                                    <p className="text-5xl font-bold mb-2 drop-shadow-lg">
                                        {wallet?.balance.toLocaleString("vi-VN") || "0"} ₫
                                    </p>
                                    <p className="text-indigo-100 text-sm">Đơn vị: {wallet?.currency || "VND"}</p>
                                </div>
                            ...
                            <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
                                    ...
                                    <button
                                        onClick={() => {
                                            if (bankAccounts.length >= 2) {
                                                setError("Mỗi người dùng chỉ được phép có tối đa 2 tài khoản ngân hàng.")
                                                return
                                            }
                                            setEditingBankAccount(null)
                                            setIsCustomBankMode(false)
                                ...
                                {bankAccounts.length >= 2 && (
                                    <div className="mb-4 p-3 bg-yellow-50 border-l-4 border-yellow-500 rounded-lg">
                                        <p className="text-sm text-yellow-800">
                                            <strong>Lưu ý:</strong> Bạn đã đạt giới hạn tối đa 2 tài khoản ngân hàng.
                                            Vui lòng xóa một tài khoản hiện có trước khi thêm mới.
                                        </p>
                                    </div>
                                )}
                                {bankAccounts.length === 0 ? (
                                    <div className="text-center py-8">
                                        ...
                                        <p className="text-gray-500 text-sm mb-4">Chưa có tài khoản ngân hàng</p>
                                        <p className="text-xs text-gray-400">Thêm tài khoản để rút tiền</p>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        {bankAccounts.map((account) => (
                                            <div
                                                key={account.id}
                                                className={`p-4 rounded-lg border-2 ${account.isDefault
                                            ...
                                            >
                                                <div className="flex items-start justify-between">
                                                    <div className="flex-1">
                                                        ...
                                                        {account.branch && (
                                                            <p className="text-xs text-gray-500 mt-1">{account.branch}</p>
                                                        )}
                                                        {account.qrCodeUrl && (
                                                            <div className="mt-2">
                                                                <Image
                            ...
                            {requests.filter(r => r.status === "pending").length > 0 && (
                                <div className="bg-yellow-50 border-l-4 border-yellow-500 rounded-lg p-6">
                                    <h2 className="text-lg font-semibold text-yellow-900 mb-4 flex items-center gap-2">
                                        ...
                                        Yêu cầu đang chờ ({requests.filter(r => r.status === "pending").length})
                                    </h2>
                                    <div className="space-y-3">
                                        {requests
                                            .filter(r => r.status === "pending")
                                            .map((request) => (
                                                <div key={request.id} className="bg-white rounded-lg p-4 border border-yellow-200 shadow-sm">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex-1">
                            ...
                            <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
                                ...
                                {transactions.length === 0 ? (
                                    <div className="text-center py-12">
                                        ...
                                        <p className="text-sm text-gray-400">Các giao dịch của bạn sẽ hiển thị ở đây</p>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        {transactions.map((tx) => (
                                            ...
                                            >
                                                <div className="flex items-center gap-4 flex-1">
                                                    <div
                                                        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-sm ${tx.type === "deposit" || tx.type === "refund"
                                                            ? "bg-green-100"
                                                            : "bg-red-100"
                                                            }`}
                                                    >
                                                ...
                                                <div className="text-right">
                                                    <p
                                                        className={`font-bold text-xl ${tx.type === "deposit" || tx.type === "refund"
                                                            ? "text-green-600"
                                                            : "text-red-600"
                                                            }`}
                                                    >
                                                    ...
                                                    <span
                                                        className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-medium ${tx.status === "success"
            ...
            {showDepositModal && (
                <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 backdrop-blur-sm overflow-y-auto">
                    <div className={`bg-white rounded-2xl ${showDepositQRCode ? 'max-w-4xl' : 'max-w-lg'} w-full p-8 shadow-2xl my-8`}>
                        ...
                        {!showDepositQRCode ? (
                                    ...
                                    <div className="flex items-start">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-yellow-800">
                                                Yêu cầu nạp tiền sẽ được gửi lên SystemAdmin để xét duyệt
                                            </p>
                                            <p className="text-xs text-yellow-700 mt-1">
                                                Sau khi chuyển tiền, SystemAdmin sẽ phê duyệt và số tiền sẽ được cộng vào ví của bạn
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <>
                                <div className="text-center mb-6">
                                    ...
                                    <p className="text-sm text-gray-600">
                                        Yêu cầu nạp tiền đã được tạo. Vui lòng quét mã QR để chuyển tiền vào tài khoản SystemAdmin
                                    </p>
                                </div>
                                    ...
                                    <div className="flex-1 space-y-4">
                                        <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-lg p-5 border-2 border-indigo-200">
                                            ...
                                            <div className="space-y-3 text-sm">
                                                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                                                    <span className="text-gray-600 font-medium w-28 flex-shrink-0">Ngân hàng:</span>
                                                    <span className="font-semibold text-gray-900">
                                                        Ngân hàng TMCP Quân Đội (MB Bank)
                                                    </span>
                                                </div>
                                                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                                                    <span className="text-gray-600 font-medium w-28 flex-shrink-0">Số tài khoản:</span>
                                                    <span className="font-semibold font-mono text-gray-900 text-lg">
                                                        {ADMIN_BANK_ACCOUNT}
                                                    </span>
                                                </div>
                                                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                                                    <span className="text-gray-600 font-medium w-28 flex-shrink-0">Chủ tài khoản:</span>
                                                    <span className="font-semibold text-gray-900">
                                                        {ADMIN_ACCOUNT_NAME}
                                                    </span>
                                                </div>
                                                <div className="flex flex-col sm:flex-row sm:items-center gap-2 pt-2 border-t border-indigo-200">
                                                    <span className="text-gray-600 font-medium w-28 flex-shrink-0">Số tiền:</span>
                                                    <span className="font-bold text-indigo-600 text-2xl">
                                                        {parseFloat(depositAmount || "0").toLocaleString("vi-VN")} ₫
                                                    </span>
                                                </div>
                                                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                                                    <span className="text-gray-600 font-medium w-28 flex-shrink-0">Nội dung:</span>
                                        ...
                                        <div className="bg-blue-50 border-l-4 border-blue-400 rounded-lg p-4">
                                            ...
                                            <ol className="text-sm text-blue-700 space-y-2 ml-6">
                                                <li className="list-decimal">Mở ứng dụng ngân hàng của bạn</li>
                                                <li className="list-decimal">Quét mã QR ở trên</li>
                                                <li className="list-decimal">Kiểm tra số tiền và nội dung chuyển khoản</li>
                                                <li className="list-decimal">Xác nhận chuyển khoản</li>
                                                <li className="list-decimal">Sau khi chuyển khoản, SystemAdmin sẽ phê duyệt và số tiền sẽ được cộng vào ví của bạn</li>
                                            </ol>
                                        </div>
                                    </div>
                                </div>
            ...
            {showWithdrawModal && (
                <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl max-w-lg w-full p-8 shadow-2xl">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-2xl font-bold text-gray-900">Rút tiền từ ví</h2>
                            <button
                                onClick={() => {
                                    setShowWithdrawModal(false)
                                    setWithdrawAmount("")
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                />
                                <p className="text-xs text-gray-500 mt-1">
                                    Số dư hiện tại: <span className="font-semibold">{wallet?.balance.toLocaleString("vi-VN") || "0"} ₫</span>
                                </p>
                            </div>
                            ...
                            {bankAccounts.length > 0 && (
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">
                                        Tài khoản ngân hàng nhận tiền <span className="text-red-500">*</span>
                                    </label>
                                    <select
                                    ...
                                    >
                                        ...
                                        {bankAccounts.map((account) => (
                                            <option key={account.id} value={account.id}>
                                                {account.bankName} - {account.accountNumber} {account.isDefault && "(Mặc định)"}
                                            </option>
                                        ))}
                                    </select>
                                    {bankAccounts.length === 0 && (
                            ...
                            {bankAccounts.length === 0 && (
                                <div className="bg-yellow-50 border-l-4 border-yellow-500 rounded-lg p-4">
                                    <p className="text-sm text-yellow-800">
                                        <strong>Lưu ý:</strong> Bạn cần thêm tài khoản ngân hàng trước khi rút tiền.
                                        SystemAdmin sẽ chuyển khoản vào tài khoản ngân hàng của bạn sau khi phê duyệt.
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="bg-white rounded-2xl max-w-2xl w-full p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="space-y-4">
                            <div className="relative">
                                ...
                                <div className="relative">
                                    <input
                                        type="text"
                                        value={bankForm.bankCode && BANKS.find(b => b.code === bankForm.bankCode)
                                            ? `${BANKS.find(b => b.code === bankForm.bankCode)?.name} (${bankForm.bankCode})`
                                            : bankSearchQuery || (bankForm.bankCode && !BANKS.find(b => b.code === bankForm.bankCode) ? bankForm.bankCode : "") || ""}
                                        onChange={(e) => {
                                        ...
                                        onFocus={() => {
                                            setShowBankDropdown(true)
                                            if (bankForm.bankCode && BANKS.find(b => b.code === bankForm.bankCode)) {
                                                setBankSearchQuery("")
                                            } else {
                                                setBankSearchQuery(bankForm.bankCode || bankForm.bankName || "")
                                            }
                                        }}
                                        onBlur={() => {
                                            ...
                                            setTimeout(() => {
                                                ...
                                                if (bankSearchQuery && !BANKS.find(b => b.code === bankSearchQuery)) {
                                    ...
                                    {showBankDropdown && (
                                        <div className="absolute z-50 w-full mt-1 bg-white border-2 border-gray-300 rounded-xl shadow-lg max-h-60 overflow-y-auto">
                                            {BANKS
                                                ...
                                                .map((bank) => (
                                                    <div
                                                        ...
                                                        className={`px-4 py-3 cursor-pointer hover:bg-indigo-50 transition-colors ${bankForm.bankCode === bank.code ? "bg-indigo-100" : ""
                                                            }`}
                                                    >
                                                        <div className="flex items-center justify-between">
                                                            <div>
                                                                <p className="font-medium text-gray-900">{bank.name}</p>
                                                                <p className="text-sm text-gray-500">Mã: {bank.code}</p>
                                                            </div>
                                            ...
                                            {bankSearchQuery && BANKS.filter(bank =>
                                                bank.name.toLowerCase().includes(bankSearchQuery.toLowerCase()) ||
                                                bank.code.includes(bankSearchQuery)
                                            ).length === 0 && (
                                                    <div className="px-4 py-3 text-sm text-gray-500">
                                                        Không tìm thấy. Bạn có thể nhập thủ công mã ngân hàng.
                                                    </div>
                                                )}
                                        </div>
                                    )}
                                ...
                                {bankForm.bankCode && BANKS.find(b => b.code === bankForm.bankCode) && (
                                    <div className="mt-2 p-3 bg-indigo-50 border border-indigo-200 rounded-lg">
                                        <p className="text-sm text-indigo-900">
                                            <span className="font-semibold">Mã ngân hàng:</span> {bankForm.bankCode}
                                        </p>
                                        <p className="text-sm text-indigo-900 mt-1">
                                            <span className="font-semibold">Tên ngân hàng:</span> {bankForm.bankName}
                                        </p>
                                    </div>
                                )}
                            </div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setBankForm({
                                                    ...bankForm,
                                                    bankCode: code,
                                                    bankName: foundBank ? foundBank.name : bankForm.bankName
                                                })
                                                if (foundBank) {
                                                    setBankSearchQuery("")
                                                    setIsCustomBankMode(false)
                            ...
                            {bankForm.bankCode && !BANKS.find(b => b.code === bankForm.bankCode) && bankForm.bankCode.length >= 6 && (
                            ...
                            {editingBankAccount && !BANKS.find(b => b.code === editingBankAccount.bankCode) && (
                                <div className="bg-yellow-50 border-l-4 border-yellow-500 rounded-lg p-4">
                                    <p className="text-sm text-yellow-800">
                                        <strong>Lưu ý:</strong> Ngân hàng này không có trong danh sách. Bạn có thể chỉnh sửa thông tin bên dưới hoặc chọn ngân hàng khác từ dropdown.
                                    </p>
                                </div>
                            )}
                        ...
                        <div className="flex gap-3 mt-6">
                            ...
                            <button
                                ...
                                className="flex-1 px-4 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium"
                            >
                                {submitting ? "Đang lưu..." : editingBankAccount ? "Cập nhật" : "Thêm mới"}
                            </button>
                        </div>
                    </div>
                </div>
