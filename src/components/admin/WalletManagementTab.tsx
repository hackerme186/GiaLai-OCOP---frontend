...
export default function WalletManagementTab() {
  ...
  const [ensuringWallets, setEnsuringWallets] = useState(false);
  ...
  const loadPendingRequestsCount = async () => {
    try {
      ...
      console.log(
        "WalletManagementTab - Pending requests count result:",
        result
      );
      ...
      console.error(
        "WalletManagementTab - Failed to load pending requests count:",
        err
      );
  ...
  const loadPendingRequests = async () => {
    try {
      ...
      console.log(
        ...
        "requests for",
        new Set(validatedRequests.map((r) => r.userId)).size,
        "users"
      );
    } catch (err) {
  ...
  const loadData = async () => {
    try {
      ...
      await Promise.all([loadPendingRequestsCount(), loadPendingRequests()]);
    } catch (err) {
  ...
  const loadUserWalletDetails = async (userId: number) => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Không thể tải chi tiết ví"
      );
    }
  };
  ...
  const handleUpdateBalance = async () => {
    ...
    try {
      ...
      // Reload data
      await Promise.all([loadData(), loadPendingRequests()]);
      if (selectedUserId) {
  ...
  const handleEnsureWallets = async () => {
    try {
      setEnsuringWallets(true);
      setError(null);
      const result = await ensureAllUserWallets();
      setSuccess(result?.message ?? "Đã kiểm tra ví cho tất cả user");
      await loadData();
    } catch (err: any) {
      console.error("Failed to ensure all wallets:", err);
      const message =
        err?.response?.message || err?.message || "Không thể tạo ví cho user";
      setError(message);
    } finally {
      setEnsuringWallets(false);
    }
  };
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Quản lý và theo dõi ví của tất cả người dùng trong hệ thống
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handleEnsureWallets}
              className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors shadow-lg disabled:opacity-60"
              disabled={ensuringWallets}
            >
              <svg
                className="w-5 h-5"
                fill="none"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-6 text-white shadow-lg">
            ...
            <p className="text-orange-100 text-xs mt-2">
              {summary.totalCustomers} Customer, {summary.totalEnterpriseAdmins}{" "}
              EnterpriseAdmin
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from(new Set(pendingRequests.map((r) => r.userId)))
              ...
              .map((userId) => {
                ...
                const validatedRequests = userRequests.filter(
                ...
                const totalAmount = validatedRequests.reduce(
                  (sum, r) => sum + r.amount,
                  0
                );
                            ...
                            <span
                              className={`px-1.5 py-0.5 rounded ${
                                userInfo.userRole === "SystemAdmin"
                                  ...
                                  : userInfo.userRole === "EnterpriseAdmin"
                    ...
                    <div className="mt-3 space-y-1">
                      {validatedRequests.map((req) => {
                        ...
                        if (req.userId !== userId) {
                          console.warn(
                            `Request ${req.id} userId mismatch: expected ${userId}, got ${req.userId}`
                          );
                          return null;
                        }
                        return (
              ...
              <tbody>
                {userWallets.map((wallet) => {
                  ...
                  const hasPendingRequests =
                    getUserIdsWithPendingRequests().has(wallet.userId);
                  const userPendingRequests = getUserPendingRequests(
                    wallet.userId
                  );

                  // Validate: ensure all requests belong to this wallet's userId
                  const validatedPendingRequests = userPendingRequests.filter(
                    (req) => req.userId === wallet.userId
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${
                            wallet.userRole === "SystemAdmin"
                              ? "bg-purple-100 text-purple-800"
                              : wallet.userRole === "EnterpriseAdmin"
                              ? "bg-blue-100 text-blue-800"
                              : "bg-green-100 text-green-800"
                          }`}
                        >
                      ...
                      <td className="px-4 py-3 text-center">
                        {hasPendingRequests &&
                        validatedPendingRequests.length > 0 ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl p-6 text-white">
                ...
                <p className="text-indigo-100 text-sm">
                  Đơn vị: {selectedWallet.currency}
                </p>
              </div>
              ...
              <div className="bg-blue-50 border-l-4 border-blue-500 rounded-lg p-4">
                <p className="text-sm text-blue-800">
                  <strong>Lưu ý:</strong> Để xem lịch sử giao dịch chi tiết, vui
                  lòng yêu cầu user đăng nhập và xem trong trang "Ví của tôi"
                  của họ.
                </p>
              </div>
            </div>
